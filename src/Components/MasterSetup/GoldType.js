...
export default function GoldType(){
...
if(result){
  ...
  SetSelectedType({
    ...
    const fetchGoldType = async () => {
    try {
    //  const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/FetchGoldType`,{ withCredentials: true});
...
useEffect(()=>{
    ...
    const UpdateForm = (event) => {
        ...
        try{
           axios.post(`${process.env.REACT_APP_API_BASE_URL}/UpdateGoldType`,SelectedType)
           .then((res)=>{SuccessMSG(res.data.message);SetSelectedType([]);fetchGoldType();}).catch((err)=> { 
    ...
    const handleDelete =(items) =>{
      try{
         axios.post(`${process.env.REACT_APP_API_BASE_URL}/DeleteGoldType`,items)
         .then((res)=>{SuccessMSG(res.data.message);fetchGoldType();}).catch((err)=> {   
    ...
    */
    ...
    <div className="MSContainer">
        <div className="MSContainerChild">
            <p className="MSCHead">Gold Type</p>
        ...
        <div className="MSContainerChild1">
            ...
            {GoldType.length <1 ? <p> No Type Found</p>:
                GoldType.map((items)=>{
