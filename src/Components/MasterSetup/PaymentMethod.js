...
if(result){
  ...
  SetSelectedType({
    ...
    try {
    //  const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/FetchPaymentType`,{ withCredentials: true});
...
useEffect(()=>{
    ...
    const UpdateForm = (event) => {
        ...
        try{
           axios.post(`${process.env.REACT_APP_API_BASE_URL}/UpdatePaymentType`,SelectedType)
           .then((res)=>{SuccessMSG(res.data.message);SetSelectedType([]);fetchPaymentType();}).catch((err)=> { 
    ...
    const handleDelete =(items) =>{
      try{
         axios.post(`${process.env.REACT_APP_API_BASE_URL}/DeletePaymentType`,items)
         .then((res)=>{SuccessMSG(res.data.message);fetchPaymentType();}).catch((err)=> {   
    ...
    const handleSubmit =  (event) =>{
        ...
        //console.log(Details);  
        try{
           axios.post(`${process.env.REACT_APP_API_BASE_URL}/AddPaymentType`,Details)
           .then((res)=> { 
            if(res.status === 200){SuccessMSG(res.data.message);}
            if(res.status === 201){ WarningMSG(res.data.message);}
            setDetails([]);fetchPaymentType();}
          ).catch((err)=> { 
            if(err.response && err.response.status !==200) FailedMSG('Something went Wrong')});
          event.target.reset();
        }catch(error){
    ...
    */
    ...
    <div className="MSContainer">
        <div className="MSContainerChild">
            <p className="MSCHead">Payment Type</p>
        ...
        <div className="MSContainerChild1">
            ...
            {PaymentType.length <1 ? <p> No Type Found</p>:
