...
if(result){
  ...
  SetSelectedType({
    ...
    const fetchReferralType = async () => {
    try {
    //  const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/FetchReferralType`,{ withCredentials: true});
      //console.log("Fetched data:", response.data);// Log the fetched data
   //   SetReferralType(response.data);
      } catch(error){
...
useEffect(()=>{
    ...
    const UpdateForm = (event) => {
        ...
        try{
           axios.post(`${process.env.REACT_APP_API_BASE_URL}/UpdateReferralType`,SelectedType)
           .then((res)=>{SuccessMSG(res.data.message);SetSelectedType([]);fetchReferralType();}).catch((err)=> { 
    ...
    const handleDelete =(items) =>{
      try{
         axios.post(`${process.env.REACT_APP_API_BASE_URL}/DeleteReferralType`,items)
         .then((res)=>{SuccessMSG(res.data.message);fetchReferralType();}).catch((err)=> {   
    ...
    const handleSubmit =  (event) =>{
        ...
        //console.log(Details);  
        try{
           axios.post(`${process.env.REACT_APP_API_BASE_URL}/AddReferralType`,Details)
           .then((res)=> { 
            if(res.status === 200){SuccessMSG(res.data.message);}
            if(res.status === 201){ WarningMSG(res.data.message);}
            setDetails([]);fetchReferralType();}
          ).catch((err)=> { 
            if(err.response && err.response.status !==200) FailedMSG('Something went Wrong')});
          event.target.reset();
        }catch(error){
    ...
    */
    ...
    <div className="MSContainer">
        <div className="MSContainerChild">
            <p className="MSCHead">Referral Type</p>
            <form className="MSForm" >
                <div className="MSFormInputs">
                <input className="MSFormInput1" type="text"  name="Type"  placeholder="Enter Type" required></input>
                <input className="MSFormInput1" type="text"  name="TypeRef" placeholder="Ref Type" required></input>
                </div>
        ...
        <div className="MSContainerChild1">
            <div className="MSCTHeads">
                <p className="MSCTHLabels">Type</p>
                <p className="MSCTHLabels">Ref Type</p>
                <div className="MSCTBtns">
                </div>
            </div>
            {ReferralType.length <1 ? <p> No Type Found</p>:
                ReferralType.map((items)=>{
